...
(defmethod change ((object omg-object) changed-obj message args)
  (if (= (change-level object) 0)
      (send-event object 'ev-change changed-obj message args)
    object))

(defmethod changing ((object omg-object) changed-obj message args)
  (if (= (change-level object) 0)
      (send-event object 'ev-changing changed-obj message args)
    object))

(defun call-with-change (body-function object function-name args)
  (let (result)
    (when (changing object object function-name args)
      (unwind-protect
          (progn (inc-change-level object)
            (setf result (funcall body-function)))
        (dec-change-level object))
      (change object object function-name args))
    result))
...
(defmethod ev-change 
           ((obj omg-object) sender changed-obj msg args)
  (change obj changed-obj msg args))

(defmethod ev-changing 
           ((obj omg-object) sender changed-obj msg args)
  (changing obj changed-obj msg args))
...
(defmethod change :after ((w window) changed-obj function args)
  (invalidate w))
